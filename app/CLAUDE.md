...
**关键文件：** `app/src/utils/markdown.ts`
...
1. **构建时脚本**：`app/scripts/ensure-article-ids.ts` - 扫描所有文章，为缺失字段的文章生成 8 位 ID
2. **开发时插件**：`app/src/plugins/blog-watcher.ts` - 自定义 Vite 插件，监听 `docs/` 目录变化，自动补全 front-matter
...
**关键文件：** `app/src/pages/NewspaperPage.vue`、`app/src/components/ui/NewspaperButton.vue`
...
**目录结构：** `app/src/composables/`

两个独立的 Canvas 动画模块：

1. **水滴涟漪** (`app/src/composables/ripple/`) - `InkBackground.vue`
2. **雪花飘落** (`app/src/composables/snowfall/`) - `SnowfallEffect.vue`
...
- `app/src/composables/common/useCanvasResize.ts` - 通用的 Canvas 尺寸调整逻辑（防抖、orientationchange 监听）
...
- `app/src/styles/base.css` - CSS Reset、基础元素样式、移动端优化
- `app/src/styles/variables.css` - CSS 变量（颜色、字体、间距等）
- `app/src/styles/common.css` - 通用样式（标签、按钮、页面标题等）
- `app/src/styles/code-block.css` - 代码块样式（配合 Shiki 高亮）
- `app/src/styles/index.css` - 全局样式入口文件
...
- `app/src/components/comments/GiscusComments.vue` - Giscus 评论组件
- `config/giscus.config.ts` - Giscus 配置文件
...
- 文章详情页底部（`app/src/pages/ArticleDetailPage.vue:18-28`）
- 自动添加分隔线区分评论区域
...
- `app/src/components/ui/SearchButton.vue` - 搜索按钮（右上角固定）
- `app/src/components/ui/SearchModal.vue` - 搜索模态框
- `app/src/utils/search.ts` - 搜索算法
...
- 使用 `import.meta.glob('@docs/**/*.md')` **别名路径**（非相对路径），确保生产构建正常加载文章

### Dock 导航菜单

**关键组件：**

- `app/src/components/ui/Dock.vue` - macOS 风格 Dock 菜单栏组件
- `app/src/constants/dock.ts` - Dock 配置文件
- `app/src/components/icons/` - 图标组件目录
...
**关键组件：** `app/src/components/ui/GitActivityChart.vue`
...
- `app/scripts/generate-sitemap.ts` - 自动生成 sitemap.xml
- 包含所有文章、首页、列表页

### 静态站点生成（SSG）

**关键文件：** `app/src/config/ssg.ts`
...
**关键文件：** `app/src/plugins/blog-watcher.ts`

开发时使用的自定义插件，实现博客文章的实时监听和自动补全 front-matter：

**监听功能：**

- 使用 `chokidar` 监听 `docs/` 目录下的所有 `.md` 文件
- 检测新文件、修改、删除事件
...
- `app/src/types/article.d.ts` - `Article` 和 `ArticleFrontMatter` 接口
- `app/src/types/ripple.d.ts` - 涟漪动画类型
- `app/src/types/snowflake.d.ts` - 雪花动画类型
- `app/src/types/git-activity.d.ts` - Git 活动数据类型
- `app/env.d.ts` - Vite 环境类型扩展（`import.meta.glob`、Vue 模块声明、ImportMetaEnv）

## 工具函数模块

**共享工具模块：**

- `app/src/utils/article-id.ts` - 文章 ID 生成和验证
  - `generateId()` - 生成 8 位随机 ID（小写字母和数字）
  - `validateId()` - 验证 ID 格式

- `app/src/utils/scan-articles.ts` - 文章扫描工具
  - `scanArticles()` - 扫描指定目录下的所有 Markdown 文件
  - `scanArticlesWithStats()` - 扫描文章并包含文件统计信息
  - `scanArticleIds()` - 扫描 docs 目录并提取文章 ID 列表

- `app/src/utils/markdown.ts` - Markdown 解析和渲染
  ...
  - **重要**：使用 `import.meta.glob('@docs/**/*.md')` 别名路径（确保生产构建正常）

- `app/src/utils/date.ts` - 日期格式化工具
  - `formatDate()` - 格式化日期（完整、简短、年月）

- `app/src/utils/dompurify.ts` - HTML 净化工具
  - `sanitizeHtml()` - 净化 HTML 内容（防止 XSS）
  - `sanitizeHtmlWithSsr()` - SSR 兼容的净化函数

- `app/src/utils/seo.ts` - SEO 优化工具
  - `useArticleSeo()` - 为文章详情页生成 SEO 元数据

- `app/src/utils/search.ts` - 文章搜索工具
...
**关键文件：** `app/src/constants/index.ts`

集中管理应用配置：

- `SITE_CONFIG` - 站点基本信息（标题、描述、作者、ICP 备案、版权信息，位于 `config/site.config.ts`）
- `ROUTES` - 路由路径常量
- `PAGINATION` - 分页配置
- `GIT_ACTIVITY` - Git 活动图配置（工作日、月份标签、提示框偏移量）
- `pwaOptions` - PWA 配置（从 `pwa.config.ts` 导入）

修改站点信息时，应更新 `config/site.config.ts` 中的 `SITE_CONFIG` 而非分散在各个文件中。

## 添加新功能时的注意事项

### 添加新的 Canvas 动画

1. 在 `app/src/composables/` 下创建新文件夹
2. 按照现有模块结构创建 `useConfig.ts`、`useAnimation.ts`、`useEvents.ts`
3. 在 `app/src/components/effects/` 创建对应的 Vue 组件
4. 使用 `app/src/composables/common/useCanvasResize.ts` 处理尺寸调整
...
2. 文章路径会自动转换为 slug（例如 `docs/category/article.md` → `category/article`）
...
1. 更新 `app/src/types/article.d.ts` 中的类型定义
2. 在 `app/src/utils/markdown.ts` 的 `parseArticle()` 函数中添加字段解析逻辑
...
- `validateSlug()` 函数检查 `../`、`./`、`\\` 等模式（`app/src/utils/markdown.ts`）
- 无效的 slug 返回 `null` 而非抛出错误
...
```typescript
// app/src/composables/useFeature.ts
...
本项目要求所有代码修改都必须同步更新相关文档，确保 `app/CLAUDE.md` 始终反映项目的真实状态。
...
- 使用清晰简洁的语言
- 提供具体的文件路径和行号（如：`app/src/utils/markdown.ts:123`）
- 包含代码示例（使用正确的语法高亮）
- 标注"重要"和"注意"事项
- 保持文档结构的一致性
...
- `NewFeature.vue` - 新功能组件（新增） ← 添加说明
  - 位置：`app/src/components/ui/NewFeature.vue`
  - 用途：实现XXX功能
  - 使用方法：见下方示例
...
- `GiscusComments.vue` - Giscus 评论系统
  - 位置：`app/src/components/comments/GiscusComments.vue`
  - 用途：为文章提供基于 GitHub Discussions 的评论功能
  - 配置：`config/giscus.config.ts`
...
- 文章列表和卡片使用 ID 进行路由跳转
- **双重保障机制**：
  - 构建时运行 `app/scripts/ensure-article-ids.ts` 批量添加 ID
...
- 必须使用**别名路径**：`import.meta.glob('@docs/**/*.md')`
- **禁止使用相对路径**：`import.meta.glob('../../docs/**/*.md')` 会在生产构建中失败
