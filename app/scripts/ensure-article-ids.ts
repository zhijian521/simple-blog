 ...
 * 处理单个文章文件
 */
function processArticle(filePath: string, docsDir: string): boolean {
    const content = fs.readFileSync(filePath, 'utf-8')
    const { attributes, body } = matter(content)

    if (attributes.id) {
        console.log(`✓ ${path.relative(docsDir, filePath)}: ${attributes.id}`)
        return false
    }

    const newId = generateId()
    writeArticle(filePath, { ...attributes, id: newId }, body)
    console.log(`+ ${path.relative(docsDir, filePath)}: ${newId}`)
    return true
}

function main() {
    const files = Array.from(walkDir(CONFIG.docsDir, '.md'))

    console.log(`扫描 ${files.length} 个文件`)

    let updatedCount = 0
    for (const filePath of files) {
        if (processArticle(filePath, CONFIG.docsDir)) {
            updatedCount++
        }
    }
