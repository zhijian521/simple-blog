...
// 常量配置
const DOCS_PATH_REGEX = /^(@docs[\\/]|.*[/\\]docs[/\\])/
...
const treeNodes = computed(() => {
    ...
    const blogModules = import.meta.glob('@docs/**/*.md', { query: '?raw' })
    const treeData: TreeNodeType[] = []

    for (const path in blogModules) {
        const relativePath = path.replace(DOCS_PATH_REGEX, '').replace(BLOG_FILE_SUFFIX, '')
