...
function toSlug(filePath: string, blogsDir: string): string {
    const relativePath = path.relative(blogsDir, filePath).replace(/\\/g, '/')
    return relativePath.replace(/\.md$/i, '')
}

export function buildArticleIndex(blogsDir: string): ArticleIndexItem[] {
    const items: ArticleIndexItem[] = []

    for (const filePath of walkDir(blogsDir, '.md')) {
        ...
        const slug = toSlug(filePath, blogsDir)
